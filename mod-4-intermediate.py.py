...
def shift_letter(letter, shift):
    if letter == " ":
        ...
        if new_ascii_code > ord("Z"):
            return chr(ord("A") + (new_ascii_code - ord("Z") - 1))
        elif new_ascii_code < ord("A"):
            return chr(ord("Z") - (ord("A") - new_ascii_code - 1))
        else:
...
def shift_by_letter(letter, letter_shift):
    if letter == " ":
        ...
        shift_index = ord(letter_shift) - ord("A")
...
def vigenere_cipher(message, key):
    ...
    for i, m in enumerate(message):
        if m == ' ':
            encrypted.append(' ')
            continue
        k = key_int[i % len(key_int)]
        encrypted.append(shift_letter(m, k))
